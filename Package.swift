// swift-tools-version: 6.2

import PackageDescription

// Before publishing: build SwAN.xcframework, zip it, upload to a GitHub Release,
// then set `url` and `checksum` (from `swift package compute-checksum SwAN.xcframework.zip`).

let package = Package(
    name: "SwAN",
    platforms: [
        .iOS(.v17),
        .macOS(.v14),
    ],
    products: [
        .library(
            name: "SwAN",
            targets: ["SwAN"]
        ),
    ],
    targets: [
        .binaryTarget(
            name: "SwAN",
            url: "https://github.com/Namalabs/SwAN/releases/download/v0.3.0/SwAN.xcframework.zip",
            checksum: "3f20740260c0dc2944e786b40b22f6ca1cb19014157a3b8c83edfa740a6ff900"
        ),
    ]
)
